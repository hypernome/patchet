...
class BootstrapTool:
    def __init__(
        self, tool: StructuredTool, args: dict | Callable[[PatchetState], dict]
    ):
        self.tool = tool
        self.args = args
...
class ReActAgent:
    ...
    def __init__(
        ...
        state_serializer_func: Callable[
            [PatchetState, list[str]], str
        ] = serialize_state,
        ...
        for ts in tool_specs:
            ...
            structured_tool = StructuredTool.from_function(
                func=ts.original_func, name=ts.name, description=ts.description
            )
        ...
        self.tool_aware_llm = init_chat_model(llm_name, temperature=0.0).bind_tools(
            tools
        )
            ...
            [
                {
                    ...
                    "content": self.prompt.format(
                        current_state=self.state_serializer_func(
                            state,
                            exclusions=(
                                self.field_exclusion_func(state)
                                if self.field_exclusion_func
                                else state.default_exclusion_list()
                            ),
                        ),
        ...
        if state.messages:
            ...
            if ai_message and isinstance(ai_message, AIMessage):
                for tool_call in ai_message.tool_calls:
                    ...
                    is_tool_an_agent: bool = tool.metadata.get("is_agent", False) or (
                        inspect.ismethod(tool.func)
                        and isinstance(tool.func.__self__, ReActAgent)
                    )
                    parent_messages = (
                        state.messages.copy() if is_tool_an_agent else None
                    )
                    args = (
                        {"state": state.model_dump(exclude=set(["messages"]))}
                        if is_tool_an_agent
                        else args
                    )
                    ...
                    try:
                        ...
                        if asyncio.iscoroutine(
                            observation
                        ) or asyncio.iscoroutinefunction(observation):
                        ...
                        state.messages.append(
                            {
                                "role": "tool",
                                "content": f"{self.safe_content(observation, tool_call_id)}",
    ...
    @traceable
    def should_continue(
        self, state: PatchetState
    ) -> Literal[Decision.CONTINUE, Decision.STOP]:
        """
            ...
            if isinstance(last_message, AIMessage) and last_message.tool_calls:
                for tool_call in last_message.tool_calls:
                    if tool_call["name"] in [
                        InternalTools.Done.__name__,
                        InternalTools.Yield.__name__,
                    ]:
                        state.messages.append(
                            {
                                "role": "tool",
    ...
    def build(self, name="", recompile: bool = False):
        ...
        start_from: str = (
            self.run_tool.__name__ if self.bootstrap_tool else self.consult_llm.__name__
        )
        graph.add_edge(START, start_from)
        graph.add_conditional_edges(
            self.consult_llm.__name__,
            self.should_continue,
            {
                Decision.CONTINUE: self.run_tool.__name__,
                Decision.STOP: self.wind_up.__name__,
            },
        )
    ...
    def safe_content(
        self, observation: any, tool_call_id: str, max_tokens: int = 500
    ) -> str:
        """
    ...
    def transfer_to_state(
        self, observation: any, state: PatchetState, parent_messages: list
    ):
        """
    ...
    def optionally_bootstrap(self, state: PatchetState):
        ...
        if self.bootstrap_tool:
            ...
            args = (
                self.bootstrap_tool.args(state)
                if callable(self.bootstrap_tool.args)
                else self.bootstrap_tool.args or {}
            )
            state.messages.append(
                AIMessage(
                    content="",
                    tool_calls=[
                        {"id": f"b:{uuid.uuid4()}", "name": tool.name, "args": args}
                    ],
                )
            )
...
class StrcturedAgent:
    ...
    def __init__(
        ...
        self.structured_llm = init_chat_model(
            llm_name, temperature=0.0
        ).with_structured_output(structure_on)

    async def structure_single_output(self, input: str):
        structured_output = await self.structured_llm.ainvoke(
            [{"role": "system", "content": self.prompt.format(input=input)}]
        )
