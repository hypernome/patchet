...
async def lifespan(app: FastAPI):
    ...
    if Path(WORKFLOW_REGISTRY_FILE).exists():
        with open(WORKFLOW_REGISTRY_FILE, "r") as workflow_registry:
            ...
            registered_workflows = DefaultDict(
                list,
                {
    ...
    with open(REGISTRY_FILE, "w") as registry:
        json.dump(
            {
                agent_id: [reg.model_dump() for reg in regs]
                for agent_id, regs in registered_agents.items()
            },
            registry,
            indent=2,
        )
    ...
    with open(WORKFLOW_REGISTRY_FILE, "w") as workflow_registry:
        json.dump(
            {
                workflow_id: [wf_reg.model_dump() for wf_reg in wf_regs]
                for workflow_id, wf_regs in registered_workflows.items()
            },
            workflow_registry,
            indent=2,
        )
...
class IntentServer:
    ...
    async def get_single_agent(
        self, app_id: str, target_agent_id: str
    ) -> List[Registration]:
        """
        Fetch agents registered with IDP for a specific app.
        """
    ...
    async def register_agent(self, registration_request: RegistrationRequest):
        ...
        for registrations in registered_agents.values():
            for registration in registrations:
                if registration.checksum == agent_checksum:
                    raise HTTPException(
                        400, "Agent with identical checksum already exists"
                    )
        ...
        registration: Registration = Registration(
            ...
            public_key=(
                ...
                if registration_request.public_key
                else None
            ),
            registered_at=round(time.time() * 1000),
        )

        registration.version = self._compute_next_checksum_version(
            agent_id=components.agent_id, registration=registration
        )
    ...
    async def mint_token(self, request: TokenRequest, has_scopes: list[str] = []):
        ...
        if request.computed_checksum != stored_checksum:
            raise HTTPException(
                401, "Agent checksum mismatch - code integrity violation"
            )
    ...
    def _compute_next_checksum_version(
        self, agent_id: str, registration: Registration
    ) -> str:
        """
        ...
        try:
            ...
            logger.warning(
                f"Invalid version format for {agent_id}: {previous_registration.version}"
            )
            return initial_version

        change_type: ChangeType = self._detect_change_type(
    ...
    def _detect_change_type(
        self, previous_registration: Registration, registration: Registration
    ) -> ChangeType:
        """
    ...
    def _validate_workflow_step(
        self, request: TokenRequest, has_scopes: list[str]
    ) -> bool:
        """
        Validate agent is allowed to execute this workflow step
        """
        workflow: list[WorkflowDefinition] = registered_workflows.get(
            request.workflow_id
        )
        if not workflow:
            return False
        ...
        if agent_id != step_def.agent:
            logger.info(
                f"The Step: {step_id} of the workflow {request.workflow_id} is supposed to be executed by {step_def.agent}, but was executed by {agent_id}"
            )
            return False
        ...
        required_step_ids: list[str] = [
            step_key
            for step_key in list(
                takewhile(
                    lambda x: steps[x].required == True and x != step_id, steps.keys()
                )
            )
        ]
        if required_step_ids:
            all_required_are_completed: bool = set(required_step_ids).issubset(
                set([s["step_id"] for s in completed_steps])
            )
        ...
        if step_def.requires_approval:
            has_approval: bool = self._check_approval(
                request.workflow_id, steps, step_id, completed_steps
            )
    ...
    def _create_intent_token(self, request: TokenRequest) -> str:
        ...
        payload = {
            ...
            "cnf": {
                "jwk": self._pem_string_to_jwk(
                    registered_agents[request.agent_id][-1].public_key
                )
            },
            "intent": {
                ...
                "delegation_chain": self._compute_sequence_hash(
                    request,
                    (
                        request.delegation_context.get("chain", [request.agent_id])
                        if request.delegation_context
                        else [request.agent_id]
                    ),
                ),
                "step_sequence_hash": self._compute_sequence_hash(
                    request,
                    (
                        request.delegation_context.get("completed_steps", [])
                        if request.delegation_context
                        else []
                    ),
                ),
            ...
            "agent_proof": {
                ...
                "registration_id": registered_agents[request.agent_id][
                    -1
                ].registration_id,
            },
        }
...
@intent_router.post(
    "/register/agent",
    dependencies=[
        Depends(require_auth(scopes=["register:intent"], audience="idp.localhost"))
    ],
)
...
@intent_router.post(
    "/register/workflow",
    dependencies=[
        Depends(require_auth(scopes=["register:workflow"], audience="idp.localhost"))
    ],
)
...
@intent_router.post(
    ...
    dependencies=[
        Depends(
            require_auth(scopes=["generate:intent-token"], audience="idp.localhost")
        )
    ],
)
...
@intent_router.get(
    "/agents/{app_id}",
    dependencies=[
        Depends(require_auth(scopes=["read:agents"], audience="idp.localhost"))
    ],
)
