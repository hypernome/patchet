...
from clientshim.secure_model import (
    AgentSpec,
    AgentIdentity,
    VerificationStatus,
    WorkflowStepStatus,
...
from util.commons import (
    ...
    to_agent_components_1,
    to_agent_components,
    _TOOLS_WITH_DEEP_CHECKSUM,
)
...
from langchain_core.tools import StructuredTool, Tool as LangChainTool
...
def secure_tool(
    name: str = None, checksum_level: ChecksumLevel = ChecksumLevel.shallow
):
    """
...
class SecureClient:
    ...
    def _record_tool_invocation(
        ...
        if status == WorkflowStepStatus.STARTED:
            ...
            logger.debug(
                f"Started step: {step_id} (agent: {agent_id}, tool: {tool_name})"
            )
            ...
            if active and active["step_id"] == step_id:
                ...
                logger.debug(
                    f"Completed step: {step_id} in {completed_step['duration']:.2f}s"
                )
            ...
            if active and active["step_id"] == step_id:
                ...
                self.workflow_state["failed_steps"] = self.workflow_state.get(
                    "failed_steps", []
                )
    ...
    def _create_workflow_tracking_wrapper(
        self, original_func: Callable, agent_id: str, tool_name: str
    ):
        """
        ...
        if asyncio.iscoroutinefunction(original_func):
            ...
            async def async_tool_wrapper(*args, **kwargs):
                ...
                self._record_tool_invocation(
                    agent_id, tool_name, WorkflowStepStatus.STARTED
                )
                ...
                try:
                    ...
                    self._record_tool_invocation(
                        agent_id, tool_name, WorkflowStepStatus.FAILED, error=str(e)
                    )
                    raise

                # Record workflow step after tool invocation.
                self._record_tool_invocation(
                    agent_id, tool_name, WorkflowStepStatus.COMPLETED
                )
    ...
    async def _register_agents_on_client(self, agent_specs: List[AgentSpec]):
        ...
        try:
            idp_registered_agents: Dict[str, Dict] = (
                await self._fetch_idp_registered_agents()
            )
        except Exception as e:
        ...
        for agent_spec in agent_specs:
            try:
                ...
                verification_results.append(
                    (agent_spec.agent_id, VerificationStatus.VERIFIED)
                )
                logger.info(f"Agent '{agent_spec.agent_id}' verified successfully")
            except Exception as e:
                verification_results.append(
                    (agent_spec.agent_id, VerificationStatus.FAILED)
                )
                logger.error(f"Agent '{agent_spec.agent_id}' verification failed: {e}")
                raise SecurityError(
                    f"Agent '{agent_spec.agent_id}' verification failed: {e}"
                )
    ...
    async def _register_agents_from_idp(
        self,
        agent_factory_function: (
            Callable[[str, str, list[LangChainTool]], Any] | None
        ) = None,
    ):
        # As a first step, fetch ground truth from IDP
        try:
            agent_registrations: Dict[str, Dict] = (
                await self._fetch_idp_registered_agents()
            )
        except Exception as e:
            logger.error(f"Failed to fetch IDP registered agents: {e}")
        ...
        # Verify that the agents provided for client registration conform with IDP.
        no_agents_as_tools: list[Dict] = [
            a
            for a in agent_registrations.values()
            if not self._contains_agents_as_tools(a)
        ]
        agents_as_tools: list[Dict] = [
            a for a in agent_registrations.values() if self._contains_agents_as_tools(a)
        ]
    ...
    def _prepare_agent(
        ...
        def to_tool_spec(tool: Dict) -> ToolSpec:
            ...
            wrapper_func = self._create_workflow_tracking_wrapper(
                original_func=tool_func, agent_id=agent_id, tool_name=tool_name
            )
        ...
        agent: ReActAgent = (
            agent_factory_function(agent_id, agent_prompt, tool_specs)
            if agent_factory_function
            else ReActAgent(
                id=agent_id, prompt=agent_prompt, tool_specs=tool_specs, limit=10
            ).build(name=agent_id, recompile=True)
        )

        # compute checksum
        agent_components = to_agent_components(agent)
        computed_checksum: str = compute_agent_checksum(
    ...
    def _verify_single_agent(
        self, agent_spec: AgentSpec, idp_registered_agents: Dict[str, Dict]
    ) -> None:
        """
        ...
        if agent_spec.agent_id not in idp_registered_agents:
            raise SecurityError(
                f"Agent '{agent_spec.agent_id}' not registered with IDP"
            )

        # Check for duplicate agent_id in current registration
        if agent_spec.agent_id in self.registered_agent_ids:
            raise SecurityError(
                f"Duplicate agent_id in registration: {agent_spec.agent_id}"
            )
        ...
        if not expected_checksum:
            raise SecurityError(
                f"No checksum found for agent '{agent_spec.agent_id}' in IDP"
            )
        ...
        agent_keys = self.agent_key_manager.agent_keys
        if agent_keys:
            current_agent_private_key: RSAPrivateKey = agent_keys[agent_spec.agent_id][
                "private_key"
            ]
            if current_agent_private_key:
                public_key: RSAPublicKey = current_agent_private_key.public_key()
        ...
        # Store verified mapping using agent class as bridge identifier
        bridge_identifier = (
            agent_spec.agent_bridge
            if isinstance(agent_spec.agent_bridge, Callable)
            else agent_spec.agent_bridge.__class__
        )
        self.bridge_to_agent[bridge_identifier] = agent_identity
        ...
        logger.debug(
            f"Agent '{agent_spec.agent_id}' registered with bridge: {bridge_identifier}"
        )
    ...
    async def _fetch_idp_registered_agents(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            registered_agents: List[Dict] = (
                agents_by_app[self.app_id] if agents_by_app else []
            )
        ...
        for frame_info in inspect.stack():
            ...
            if "self" in frame.f_locals:
                ...
                if bridge_identifier in self.bridge_to_agent:
                    ...
                    logger.debug(
                        f"Detected agent context via bridge identifier: {agent_identity.agent_id}"
                    )
            ...
            if func is not None and func in self.tool_to_agent:
                ...
                running_agent_instances: list[ReActAgent] = (
                    self._find_react_agent_instances([func])
                )
                if running_agent_instances:
                    ...
                    for i in running_agent_instances:
                        ...
                        runtime_checksum: str = compute_agent_checksum(
                            agent_components=agent_comps
                        )
                        if runtime_checksum not in [
                            a_id.checksum for a_id in agent_identities
                        ]:
                            raise SecurityError(
                        ...
                        agent_identity = next(
                            (
                                a_id
                                for a_id in agent_identities
                                if runtime_checksum == a_id.checksum
                                and running_agent_id == a_id.agent_id
                            ),
                            None,
                        )
                        if agent_identity:
                            logger.debug(
                                f"Detected agent context via function name: {agent_identity.agent_id}"
                            )
                            return agent_identity
                    raise SecurityError(
                        "The currently running agent could not be verified with any of the registered agents."
                    )
    ...
    def _find_react_agent_instances(
        self, containing_tools: list[Callable] = None
    ) -> list[ReActAgent]:
        """
        ...
        try:
            agents: list[ReActAgent] = [
                obj for obj in gc.get_objects() if isinstance(obj, ReActAgent)
            ]
            if containing_tools:
                ...
                return [
                    ...
                    for agent in agents
                    if any(
                        func in [ts.original_func for ts in agent.real_tool_specs()]
                        for func in containing_tools
                    )
                ]
            return agents
        except Exception as e:
        ...
        try:
            async with self.authenticated_request(
                ...
                response = await client.post(
                    url=f"{self.idp_url}/intent/token", json=token_request
                )
                response.raise_for_status()
            ...
            raise SecurityError(
                f"Token request failed: {e.response.status_code} - {error_detail}"
            )
        except Exception as e:
        ...
        try:
            ...
            if mode == AuthMode.intent:
                ...
                cache_key = self._get_cache_key(
                    workflow_id, agent_identity.agent_id, audience, "_".join(scopes)
                )
            else:
            ...
            with self._cache_lock:
                ...
                if cached_token and self._is_token_valid(cached_token):
                    ...
                    logger.debug(
                        f"Using cached token for {agent_identity.agent_id if mode == AuthMode.intent else self.app_id}"
                    )
                else:
                    cached_token = None
            ...
            if not cached_token:
                token_response: TokenResponse = (
                    ...
                    if mode == AuthMode.intent
                    else await self._mint_oauth_token(
                        auth_profile_name, scopes, audience
                    )
                )
                ...
                logger.debug(
                    f"Minted fresh token for {agent_identity.agent_id if mode == AuthMode.intent else self.app_id}"
                )
            ...
            if pop_data and is_intent_mode_on():
                ...
                pop_signature = agent_identity.private_key.sign(
                    pop_message, padding=padding.PKCS1v15(), algorithm=hashes.SHA256()
                )
    ...
    async def _mint_oauth_token(
        self, auth_profile_name: AuthProfileName, scopes, audience: str = None
    ):
        """
        ...
        token_response = await self._http_client.post(
            url=f"{self.idp_url}/oauth/token", data=token_request, headers=headers
        )
...
def _secure_factory():
    ...
    async def init_security(
        ...
        if not _secure_client:
            _secure_client = SecureClient(
                app_id=app_id, idp_url=idp_url, agent_specs=agent_specs
            )
    ...
    def get_secure_client() -> SecureClient:
        if not _secure_client:
            raise RuntimeError(
                "SecureClient is not initialized yet. Call init_security() first."
            )
