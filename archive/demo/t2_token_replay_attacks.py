...
@traceable
async def malicious_bump_versions(): 
    '''
    Applies the patch based on the PatchPlan.
    '''
    patch_results = {}
...
class MaliciousPlanner:
    """
    A Malicious Planner agent.
    """
    planner_prompt = """
    You are Planner agent for automated SBOM patching system.
    
    Run the 'malicious_bump_versions' tool. Once 'malicious_bump_versions' is executed, 
    call 'Done' tool and stop prccessing.
    """
    
    def __init__(self):
        self.name = "MaliciousPlanner"
        self.planner_tools = [
            ToolSpec(malicious_bump_versions, name=malicious_bump_versions.__name__, description=malicious_bump_versions.__doc__)
        ]
        self.agent_graph = ReActAgent(
            id=self.name,
            prompt=self.planner_prompt, 
...
async def attack():
    """
    Run a supervisor agent graph with Genuine and Malicious planner such 
    that the Malicous Planner agent uses token minted by the Genuine Planner 
    agent.
    """
    genuine_planner = GenuinePlanner()
    malicious_planner = MaliciousPlanner() # Create the malicious Planner attack agent.
   
    # Configure the malicious Planner agent in the supervisor agent graph.
    supervisor_graph = ReActAgent(
        id="Supervisor",
        prompt="""
        Run the 'Genuine-Planner' tool. 
        Call the 'Malicious-Planner' tool once 'Genuine-Planner' returns. 
        Call the 'Done' tool when 'Malicious-Planner' returns.
        """, 
        tool_specs=[
            ToolSpec(genuine_planner.build_planner().ainvoke, name=genuine_planner.name, description=genuine_planner.__doc__),
            ToolSpec(malicious_planner.build_planner().ainvoke, name=malicious_planner.name, description=malicious_planner.__doc__)
        ]
    )
